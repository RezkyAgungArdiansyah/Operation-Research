...
while true
    ...
    Matrices(end+1,:) = zeros(1,width(Matrices));
    for row = 1:nConstraint
        if mod(Matrices(row,end),1) ~= 0
            cut = (Matrices(row,nVariable+1:end) - floor(Matrices(row,nVariable+1:end))).*-1;
            break;
        end
    end
    Matrices(end,[nVariable+1:end]) = cut;
    Matrices = [Matrices(:,1:end-1) zeros(height(Matrices),1) Matrices(:,end)];
    Matrices(end,end-1) = 1;
    Z = [Z(1:end-1) 0 Z(end)];
    header = [header(1:end-1) "s"+string(nConstraint+1) header(end)];
    RowNames = [RowNames(1:end-1); "s"+string(nConstraint+1) ;RowNames(end)];
    ...
    format short;
    format rat;
    BarisKunci = height(Matrices);
    disp(row);
    format short;
    compare_smallest = abs(Z(fixnVar+1:end-2)./Matrices(row,fixnVar+1:end-2));
    disp(compare_smallest);
    format rat;
    VariableKunci = input("Choose column key : ");
    VariableKunci = VariableKunci + fixnVar;
    V(BarisKunci) = VariableKunci;
    Matrices(BarisKunci,:) = Matrices(BarisKunci,:)./Matrices(BarisKunci,VariableKunci);
    for j = 1 : nConstraint
        if j ~= BarisKunci
    ...
    while true
        NegativeRHS = find(Matrices(:,end) < 0) 
        if isempty(NegativeRHS)
            break
        end
        BarisKunci = NegativeRHS(1);
        format short;
        compare_smallest = abs(Z(fixnVar+1:end-2)./Matrices(BarisKunci,fixnVar+1:end-2));
        disp(compare_smallest);
        format rat;
        VariableKunci = input("Choose column key : ");
        VariableKunci = VariableKunci + fixnVar;
